...
public class BPlusTree {
    ...
    private void insertNonUnique(BPlusNode leaf, String key, String docId) {
        for (int i = 0; i < leaf.keys.size(); i++) {
            if (compareKeys(leaf.keys.get(i), key) == 0) {
    ...
    public List<String> search(String key) {
        ...
        for (int i = 0; i < node.keys.size(); i++) {
            if (compareKeys(node.keys.get(i), key) == 0) {
                if (unique) {
                    return new ArrayList<>(List.of(node.docIds.get(i)));
                } else {
                    return new ArrayList<>(node.docIdsList.get(i));
                }
            }
        }
